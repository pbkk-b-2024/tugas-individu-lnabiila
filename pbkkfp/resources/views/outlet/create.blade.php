<x-guest-layout>
    ...
    <form method="POST" action="{{ route('outlet') }}">
        @csrf
        <div>
        ...
        <div class="mt-4">
            ...
            <textarea id="address" name="address" rows="5" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full"></textarea>
            <x-input-error :messages="$errors->get('address')" class="mt-2" />
        </div>
        <div>
            <x-input-label for="open_hour" :value="__('Open Hour')" />
            <x-text-input id="open_hour" class="block mt-1 w-full" type="time" name="open_hour" :value="old('open_hour')" required autofocus autocomplete="open_hour" />
            <x-input-error :messages="$errors->get('open_hour')" class="mt-2" />
        </div>
        <div>
            <x-input-label for="close_hour" :value="__('Close Hour')" />
            <x-text-input id="close_hour" class="block mt-1 w-full" type="time" name="close_hour" :value="old('close_hour')" required autofocus autocomplete="close_hour" />
